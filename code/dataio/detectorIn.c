...
void detector_module(double **x_ptr, double *detector, int *detcellid, char *detector_name, int max_blocks, int shot_block_size){
    ...
    for (int i=0;i<N_detector_geometry_x/2;i++) printf("\t detectorx %d: [%lf %lf] mm\n",i,detector_geometry_x[2*i],detector_geometry_x[2*i+1]);
    for (int j=0;j<N_detector_geometry_y/2;j++) printf("\t detectory %d: [%lf %lf] mm\n",j,detector_geometry_y[2*j],detector_geometry_y[2*j+1]);
