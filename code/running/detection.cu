...
__device__ int calculate_detection_position(double *X, double *X_prev, double *detector_geometry){
    ...
    if((detector_distance*detector_distance_prev<=0) && (X[3]>0)){
        ...
        X_new[0] = hermite_spline(X_prev[0], X[0], X_prev[3]*dt, X[3]*dt, detector_distance_rate);
        X_new[1] = hermite_spline(X_prev[1], X[1], X_prev[4]*dt, X[4]*dt, detector_distance_rate);
        X_new[2] = hermite_spline(X_prev[2], X[2], X_prev[5]*dt, X[5]*dt, detector_distance_rate);
