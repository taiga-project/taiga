...
void detector_module(double **x_ptr, double *detector, int *detcellid, char *detector_name, int max_blocks, int shot_block_size, int number_of_particles, char *export_folder, char *runnumber){
    ...
    if (( N_dgx > 0) && ( N_dgy > 0)) {
        ...
        export_data(DG,N_dg,export_folder,concat(runnumber,"/detector"),"cellcounter.dat",N_dgy/2);
        CopyFile(concat("input/detector/", detector_name, "/detx"), concat(export_folder,"/",runnumber,"/detector/detx.dat"), 0);
        CopyFile(concat("input/detector/", detector_name, "/dety"), concat(export_folder,"/",runnumber,"/detector/dety.dat"), 0);
    }else{
