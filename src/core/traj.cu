...
__device__ void (*solve_diffeq)(double *X, double *X_prev, double *B, double *E, double *E_prev, double eperm, double timestep);
...
__device__ int calculate_trajectory(TaigaCommons *c, double X[6], int detcellid){
    ...
    for (int loopi=0; (loopi < c->max_step_number && (detcellid == CALCULATION_NOT_FINISHED)); ++loopi){
        ...
        (*solve_diffeq)(X, X_prev, local_bfield, local_efield, local_efield_prev, eperm, timestep);
