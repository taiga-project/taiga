...
struct LinearInterpolation {
    ...
    static int apply(double *X, double *X_prev,
                     ...
                     double timestep)
    {
        ...
        if ((detector_distance * detector_distance_prev <= 0) && (X[3] > 0)) {
            for (int i = 0; i < 6; ++i)
                X[i] = interpolate(X_prev[i], X[i], 0.0,
                                   detector_distance_prev, detector_distance);

            X[TIME_OF_FLIGHT_ID] += interpolate(0.0, 1.0, 0.0,
                                                detector_distance_prev,
                                                detector_distance) * timestep;
...
struct HermiteInterpolation {
                     ...
                     double timestep)
    {
        ...
        if ((detector_distance * detector_distance_prev <= 0) && (X[3] > 0)) {
            ...
            interpolate_hermite(X_prev, X, detector_plane, timestep);
